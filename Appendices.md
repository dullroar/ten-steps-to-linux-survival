...
* [**`$?`**](http://linux.die.net/abs-guide/exit-status.html)\drenv{Question mark} - the exit
code for the last command or program executed.

* [**`$PATH`**](http://linux.die.net/Bash-Beginners-Guide/sect_03_02.html)\drenv{PATH} -
...
* **`&&`**\index{And operator} - execute the second
command only if the first command succeeds.

* **`||`**\index{Or operator} - execute the secon
...
* **`stderr`**\index{stderr} - file descriptor 2, always open for writing from a
process, defaults to the screen on a terminal session.

* **`stdin`**\index{stdin} - file descriptor 0, always open for reading in a
process, defaults to the keyboard input on a terminal session.

* **`stdout`**\index{stdout} - file descriptor 1, always open for writing from a
process, defaults to the screen on a terminal session.
...
* [**`~`**](http://linux.die.net/Bash-Beginners-Guide/sect_03_04.html)\index{home} -
shortcut for current user's home directory.

* **`.bash_history`**\index{.bash\_history} - history of commands entered at the command
...
* **`/etc`**\index{etc} - configuration files location.

* **`/home`**\index{home} - "home" or user profile directories.

* **`/proc`**\index{proc} - system run-time information.

* **`/root`**\index{root} - "home" directory for "root" user (system admin).

* **`/tmp`**\index{tmp} - temporary files location.

* **`/var/log`**\index{log} - log files location.
...
* [**`7z`**](http://linux.die.net/man/1/7z)\drcmd{7z} - compress and uncompress files
and directories using the 7-zip algorithm.

* [**`apropos`**](http://linux.die.net/man/1/apropos)\drcmd{apropos} - search for help on
commands by pattern.

* [**`awk`**](http://linux.die.net/man/1/awk)\drcmd{awk} - language for processing
streams of data.

* [**`bash`**](http://linux.die.net/man/1/bash)\drcmd{bash} - the Bourne-again shell.

* [**`bzip2`**](http://linux.die.net/man/1/bzip2)\drcmd{bzip2} - compress and uncompress
files using the `bzip2` algorithm.

* [**`cat`**](http://linux.die.net/man/1/cat)\drcmd{cat} - concatenate the input files to
`stdout`.

* [**`cd`**](http://linux.die.net/man/1/cd)\drcmd{cd} - change the current directory.

* [**`chgrp`**](http://linux.die.net/man/1/chgrp)\drcmd{chgrp} - change the primary group of a
file or directory.

* [**`chmod`**](http://linux.die.net/man/1/chmod)\drcmd{chmod} - change the permissions (mode
...
* [**`crontab`**](http://linux.die.net/man/1/crontab)\drcmd{crontab} - display or edit tasks to
be run by `cron`.

* [**`curl`**](http://linux.die.net/man/1/curl)\drcmd{curl} - download files from the
internet.

* [**`df`**](http://linux.die.net/man/1/df)\drcmd{df} - show space utilization by
file system.

* [**`diff`**](http://linux.die.net/man/1/diff)\drcmd{diff} - show the differences between
files.

* [**`dig`**](http://linux.die.net/man/1/dig)\drcmd{dig} - look up DNS info on an address.

* [**`dpkg`**](http://linux.die.net/man/1/dpkg)\drcmd{dpkg} - package manager for
Debian flavors.

* [**`echo`**](http://linux.die.net/man/1/echo)\drcmd{echo} - display passed parameters to
`stdout`.

* [**`email`**](http://linux.die.net/man/1/email)\drcmd{email} - send email.

* [**`false`**](http://linux.die.net/man/1/false)\drcmd{false} - do nothing, unsuccessfully.

* [**`file`**](http://linux.die.net/man/1/file)\drcmd{file} - give best guess as to type of
file.

* [**`find`**](http://linux.die.net/man/1/find)\drcmd{find} - find files based on
...
* [**`help`**](http://linux.die.net/man/1/help)\drcmd{help} - help for built-in commands in
`bash`.

* [**`info`**](http://linux.die.net/man/1/info)\drcmd{info} - an alternative for `man`,
especially for GNU programs. Remember `q` quits.

* [**`less`**](http://linux.die.net/man/1/less)\drcmd{less} - display the file one page at a
time on `stdout`.

* [**`ln`**](http://linux.die.net/man/1/ln)\drcmd{ln} - create hard or soft (shortcut)
links.

* [**`ls`**](http://linux.die.net/man/1/ls)\drcmd{ls} - list directory contents.

* [**`man`**](http://linux.die.net/man/1/man)\drcmd{man} - display manual pages. Remember `q`
quits.

* [**`mkdir`**](http://linux.die.net/man/1/mkdir)\drcmd{mkdir} - make a new directory.

* [**`mutt`**](http://linux.die.net/man/1/mutt)\drcmd{mutt} - email client.

* [**`mv`**](http://linux.die.net/man/1/mv)\drcmd{mv} - move files or directories.

* [**`pine`**](http://linux.die.net/man/1/pine)\drcmd{pine} - email client.

* [**`ps`**](http://linux.die.net/man/1/ps)\drcmd{ps} - list running processes.

* [**`pwd`**](http://linux.die.net/man/1/pwd)\drcmd{pwd} - print the current (working)
directory name.

* [**`rename`**](http://linux.die.net/man/1/rename)\drcmd{rename} - rename files in more complex
ways than `mv` can.

* [**`rm`**](http://linux.die.net/man/1/rm)\drcmd{rm} - delete (remove) files or
directories.

* [**`set`**](http://linux.die.net/man/1/set)\drcmd{set} - set an environment variable, or
display all environment variables.

* [**`sort`**](http://linux.die.net/man/1/sort)\drcmd{sort} - sort `stdin` or a file to
`stdout`.

* [**`ssh`**](http://linux.die.net/man/1/ssh)\drcmd{ssh} - secure shell termina
progam and protocol.

* [**`tail`**](http://linux.die.net/man/1/tail)\drcmd{tail} - display the last lines of
a file.

* [**`tar`**](http://linux.die.net/man/1/tar)\drcmd{tar} - "tape archive", a way to
combine directories into a single flat file.

* [**`tee`**](http://linux.die.net/man/1/tee)\drcmd{tee} - write to a file and
`stdout` at the same time.

* [**`telnet`**](http://linux.die.net/man/1/telnet)\drcmd{telnet} - ancient terminal
program and protocol.

* [**`touch`**](http://linux.die.net/man/1/touch)\drcmd{touch} - create an empty file or
...
* [**`unzip`**](http://linux.die.net/man/1/unzip)\drcmd{unzip} - uncompress `.zip`
files.

* [**`vi`**](http://linux.die.net/man/1/vi)\drcmd{vi} - "visual" editor, a file
editor.

* [**`wget`**](http://linux.die.net/man/1/wget)\drcmd{wget} - download files from the
internet.

* [**`whoami`**](http://linux.die.net/man/1/whoami)\drcmd{whoami} - the answer to life's
most existential question.

* [**`whois`**](http://linux.die.net/man/1/whois)\drcmd{whois} - look up DNS ownership
info on an address.

* [**`zip`**](http://linux.die.net/man/1/zip)\drcmd{zip} - compress files and
...
* [**`apt-get`**](http://linux.die.net/man/8/apt-get)\drcmd{apt-get} - package manager for
Debian flavors.

* [**`aptitude`**](http://linux.die.net/man/8/aptitude)\drcmd{aptitude} - package manager for
Debian flavors.

* [**`cron`**](http://linux.die.net/man/8/cron)\drcmd{cron} - system for running
"scheduled tasks."

* [**`dmesg`**](http://linux.die.net/man/8/dmesg)\drcmd{dmesg} - display kernel log
messages.

* [**`ifconfig`**](http://linux.die.net/man/8/ifconfig)\drcmd{ifconfig} - display network
(interface) configuration.

* [**`mount`**](http://linux.die.net/man/8/mount)\drcmd{mount} - mount a file system to
a specific location.

* [**`ping`**](http://linux.die.net/man/8/ping)\drcmd{ping} - test for network
connectivity to an IP address.

* [**`reboot`**](http://linux.die.net/man/8/reboot)\drcmd{reboot} - restart the system.

* [**`rpm`**](http://linux.die.net/man/8/rpm)\drcmd{rpm} - package manager for Fedora
flavors.

* [**`shutdown`**](http://linux.die.net/man/8/shutdown)\drcmd{shutdown} - shutdown or
restart the system.

* [**`sudo`**](http://linux.die.net/man/8/sudo)\drcmd{sudo} - execute a command with
elevated privileges.

* [**`traceroute`**](http://linux.die.net/man/8/traceroute)\drcmd{traceroute} - trace the
...
Remembering that `&&`\index{And operator} only executes the next command if the prior one is
...
like `if`/`fi`\drcmd{if}. But simple scripts that just "do things" in a certain
order are within scope, and the following, which installs
[`freerdp`](https://github.com/freerdp/freerdp)\drcmd{freerdp}, is a good example of
simply taking the guesswork out of doing something repetitive across
multiple machines. I keep this `installrdp` script in Dropbox so I can run it
quickly and easily on any new machine I set up (once I get Dropbox set
up on the machine!)
