...
[Multics](https://en.wikipedia.org/wiki/Multics)\drios{Multics} at MIT, GE and Bell Labs.
...
[UNIX](https://en.wikipedia.org/wiki/History_of_Unix)\drios{UNIX} is born.

* **More trouble from Berkeley** - late 1970s, Carter, disco, Iran hostages, UC Berkely releases
the [Berkeley Software
Distribution](https://en.wikipedia.org/wiki/Berkeley_Software_Distribution)
(BSD)\drios{BSD}, a port based on the Bell Labs UNIX. Let the forking begin!

* **UNIX goes commercial** - 1980s, Reagan, Iran Contra, *E.T.*, AT&T releases [System
V](https://en.wikipedia.org/wiki/UNIX_System_V)\drios{System V} as first commercial UNIX. From the
same background as Bell Labs UNIX, System V evolved with subtle and not so subtle differences in approaches to command syntax, networking and much more. It is this release and AT&T's copyrights
...
proliferation of proprietary (and different) "UNIX" platforms:
    ...
    * **Sun SunOS**\drios{SunOS} - BSD flavor.
    * **Sun Solaris**\drios{Solaris} - System V flavor. Now Oracle Solaris.
    * **IBM AIX**\drios{AIX}
    * **SGI IRIX**\drios{IRIX}
...
[Linux](https://en.wikipedia.org/wiki/Linux)\drios{Linux} based on
[MINIX](https://en.wikipedia.org/wiki/MINIX)\drios{MINIX} (and hence why Linus says Linux is
pronounced like "MINIX" and not like "Linus").
...
and [OpenBSD](https://en.wikipedia.org/wiki/OpenBSD)\drios{OpenBSD}. All happen in the same time
frame as Linux. Like Linux distros, each has its own focus and prejudices, some of which are
distinctly "anti-Linux." The "big three" are all still in heavy use today, especially among ISPs.
The perception is still out there among a generation of sysadmins that Linux is for the desktop and
BSDs for servers, but that reality shifted a long time ago.
...
* [**POSIX**](https://en.wikipedia.org/wiki/POSIX)\drios{POSIX} - a de jure set of standards
created in the 1980s and 1990s to try to bring order to the chaos that was commercial UNIX-flavored
operating systems of the time. It worked. Sorta. Especially once the US government started
...
* [**GNU Project**](https://en.wikipedia.org/wiki/GNU_Project)\drios{GNU} - [Richard
...
It is similar to the history and relationship between `COMMAND.EXE`\drshl{COMMAND.EXE} in DOS and
`CMD.EXE`\drshl{CMD.EXE} in Windows 10, where this would work in both:
...
[`ps`](http://linux.die.net/man/1/ps)\drcmd{ps} (*process*) command on a Linux system, in this case
Linux Mint under `bash`\drshl{bash}:
...
Versus the "same" command on a FreeBSD\drios{FreeBSD} system at my ISP, where `csh`\drshl{csh} is
the default shell:
...
this and putting it all together falls to distributions or "distros."\drios{Linux distros} These
have evolved over time into a series of
...
choice and Debian my preferred server OS, both based on familiarity.

* **`pacman`**\drcmd{pacman}\index{Package management!\texttt{pacman} command} -
[Arch](https://en.wikipedia.org/wiki/Arch_Linux)\drios{Arch} flavors.

* **`rpm`\drcmd{rpm}\index{Package management!\texttt{rpm} command} and
`yum`\drcmd{yum}\index{Package management!\texttt{yum} command}** - Red Hat\drios{Red Hat} flavors,such as
[Fedora](https://en.wikipedia.org/wiki/Fedora_%28operating_system%29)\drios{Fedora},
[Red Hat
Enterprise](https://en.wikipedia.org/wiki/Red_Hat_Enterprise_Linux)\drios{Red Hat Enterprise Linux (RHEL)}
...
[Slackware](https://en.wikipedia.org/wiki/Slackware)\drios{Slackware}, some others.
...
"looks like" UNIX at some level.
[BusyBox](https://en.wikipedia.org/wiki/BusyBox)\drios{BusyBox}\drshl{BusyBox} is a good example of
a "UNIX-like" shell (command prompt) used by many embedded systems. Of course, as to what's
actually available, who knows? If you can get shell open, the best thing to do is see what works.

## Cygwin{.unnumbered}

[Cygwin](http://cygwin.com/)\drios{Cygwin} is an interesting beast. It is a DLL for Windows that
implements most of the POSIX and related UNIX-like "system API calls" for programming, and then is
also a series of ported open source packages, including shells, utilities and even desktop
environments, all ***recompiled*** to run on Windows as long as the Cygwin DLL is accessible. Like
a Linux distro it has an installer that is a "package manager," and if a package isn't available,
