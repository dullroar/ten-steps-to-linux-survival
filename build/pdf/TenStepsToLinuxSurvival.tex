...
the console, it is reading from \emph{stdin}. \drcmd{echo}
...
seen on the console in real life: \drcmd{cat}
...
\item
  ...
  slashes (\texttt{/}) before sending it into... \drcmd{tr}
\item
  A \texttt{while} loop that reads each line into a variable called
  \texttt{\$line} and then calls... \drcmd{while}
\item
...
\texttt{view}{]}(http://linux.die.net/man/1/view), the alias for
...
(change) command. \drcmd{sort} \drvic{"!}{invoke external command}
...
\href{http://linux.die.net/man/1/fmt}{\texttt{fmt}} command. \drcmd{fmt}
...
\texttt{nano}{]}(http://linux.die.net/man/1/nano). You can see the
