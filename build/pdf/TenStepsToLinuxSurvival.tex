...
\DeclareRobustCommand{\fimg}[3]{\ifxetex\begin{figure}[H]\caption{#2}\label{fig:#3}\includegraphics{#1}\end{figure}\fi}
...
\ldots{}versus on a FreeBSD\drios{FreeBSD} system at my ISP, where
...
distributions or ``distros.''\drios{Linux distros} These have evolved
...
\item
  ...
  \item
    ...
    - Bourne-again shell (get it?), the ``standard'' Linux shell (as
    much as anything is standard across Linux distros).
  \item
...
Look at the output of the \texttt{PATH}\drenv{PATH} environment variable
under \texttt{bash}:
...
from the command prompt, so \textbf{\emph{BE CAREFUL!}}
...
\texttt{/root} for the ``root'' (admin) id. To change to a user's
``home'' directory, simply use \texttt{cd} with no parameters:
...
``UNIX'' is with
\href{http://linux.die.net/man/1/tar}{\texttt{tar}}\drcmd{tar}, which is
so old that \texttt{tar} stands for ``tape archive.'' Its purpose is to
take virtually any directory structure and create a single output
``stream'' or file of it. That is then typically ran through a
...
So, both hard\index{hard links} and soft links\index{soft links} can
