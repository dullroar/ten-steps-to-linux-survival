...
\clearpage
~\\\hspace*{0.333em}\\ By James Lehmer ~\\\hspace*{0.333em}\\ v0.8
~\\\hspace*{0.333em}\\
...
\KeywordTok{~} \NormalTok{$ }\KeywordTok{ps} \NormalTok{-AH}
  \KeywordTok{PID} \NormalTok{TTY          TIME CMD}
    ...
    \KeywordTok{7} \NormalTok{?        00:00:19   rcu_sched}
    \KeywordTok{8} \NormalTok{?        00:00:04   rcuos/0}
    \KeywordTok{9} \NormalTok{?        00:00:09   rcuos/1}
   \KeywordTok{10} \NormalTok{?        00:00:07   rcuos/2}
...
\section*{bash Built-Ins}\label{bash-built-ins}
\addcontentsline{toc}{section}{bash Built-Ins}
...
\OtherTok{PATHEXT=}\NormalTok{.COM;}\KeywordTok{.EXE;.BAT;.CMD;.VBS;.VBE;.JS;.JSE;.WSF;.WSH;.MSC}
...
\KeywordTok{xext-dev} \NormalTok{libxinerama-dev \textbackslash{}}
  \KeywordTok{libxcursor-dev} \NormalTok{libxdamage-dev libxv-dev libxkbfile-dev libasound2-dev libcups2}
  \KeywordTok{-dev} \NormalTok{libxml2 libxml2-dev \textbackslash{}}
  \KeywordTok{libxrandr-dev} \NormalTok{libgstreamer0.10-dev libgstreamer-plugins-base0.10-dev libxi-dev}
...
\itemsep1pt\parskip0pt\parsep0pt
\item
  \textbf{r} - 2\textsuperscript{2} = 4
\item
  \textbf{w} - 2\textsuperscript{1} = 2
\item
  \textbf{x} - 2\textsuperscript{0} = 1
\item
  \textbf{-} - 0
\end{itemize}
...
\item
  \textbf{-{}-{}-} - 0 + 0 + 0 = 0
\item
  \textbf{r-{}-} - 2\textsuperscript{2} + 0 + 0 = 4
\item
  \textbf{r-x} - 2\textsuperscript{2} + 0 + 2\textsuperscript{0} = 5
\item
  \textbf{rw-} - 2\textsuperscript{2} + 2\textsuperscript{1} + 0 = 6
\item
  \textbf{rwx} - 2\textsuperscript{2} + 2\textsuperscript{1} +
  2\textsuperscript{0} = 7
\end{itemize}
...
\section*{What's the diff?}\label{whats-the-diff}
\addcontentsline{toc}{section}{What's the diff?}
...
\KeywordTok{~} \NormalTok{$ }\KeywordTok{find} \NormalTok{//myserver/myshare/logs/000[4-9] -name }\DataTypeTok{\textbackslash{}*}\NormalTok{.dat -newer logchecker.csv \textbackslash{}}
    \KeywordTok{-exec} \NormalTok{/home/myuser/Sandbox/FileCheckers/logchecker }\DataTypeTok{\textbackslash{}\{\textbackslash{}\}} \DataTypeTok{\textbackslash{};}
\end{Highlighting}
\end{Shaded}
...
\section*{Useful find Options}\label{useful-find-options}
\addcontentsline{toc}{section}{Useful find Options}
...
\section*{Useful find Actions}\label{useful-find-actions}
\addcontentsline{toc}{section}{Useful find Actions}
...
\section*{Groveling With grep}\label{groveling-with-grep}
\addcontentsline{toc}{section}{Groveling With grep}
...
\KeywordTok{~} \NormalTok{$ }\KeywordTok{find} \NormalTok{. ! -name pdfinfo.log -newer pdfinfo.log -type f -exec grep -H \textbackslash{}}
    \KeywordTok{-i} \NormalTok{pdfinfo }\DataTypeTok{\textbackslash{}\{\textbackslash{}\}} \DataTypeTok{\textbackslash{};} \KeywordTok{>} \NormalTok{pdfinfo.log}
\end{Highlighting}
\end{Shaded}
...
\section*{Gawking at awk}\label{gawking-at-awk}
\addcontentsline{toc}{section}{Gawking at awk}
...
\hyperdef{}{vi}{\chapter{vi}\label{vi}}
...
\section*{Circumnavigating vi}\label{circumnavigating-vi}
\addcontentsline{toc}{section}{Circumnavigating vi}
...
\KeywordTok{email} \NormalTok{--blank-mail --subject }\StringTok{"Possibly corrupted files found..."} \NormalTok{\textbackslash{}}
  \KeywordTok{--smtp-server} \NormalTok{smtp --attach badfiles.csv --from-name NoReply \textbackslash{}}
  \KeywordTok{--from-addr} \NormalTok{noreply@mycorp.com alert@mycorp.com}
\end{Highlighting}
\end{Shaded}
...
man?}\label{how-do-you-know-what-you-dont-know-man}}
...
\section*{man, is that info apropos?}\label{man-is-that-info-apropos}
\addcontentsline{toc}{section}{man, is that info apropos?}
...
\section*{How Do You Google, man?}\label{how-do-you-google-man}
\addcontentsline{toc}{section}{How Do You Google, man?}
...
\section*{Which which is Which?}\label{which-which-is-which}
\addcontentsline{toc}{section}{Which which is Which?}
...
\KeywordTok{sudo} \NormalTok{apt-get -y install build-essential git-core cmake libssl-dev \textbackslash{}}
  \KeywordTok{libx11-dev} \NormalTok{libxext-dev libxinerama-dev libxcursor-dev libxdamage-dev \textbackslash{}}
  \KeywordTok{libxv-dev} \NormalTok{libxkbfile-dev libasound2-dev libcups2-dev   libxml2 \textbackslash{}}
  \KeywordTok{libxml2-dev} \NormalTok{libxrandr-dev libgstreamer0.10-dev \textbackslash{}}
  \KeywordTok{libgstreamer-plugins-base0.10-dev} \NormalTok{libxi-dev \textbackslash{}}
  \KeywordTok{libgstreamer-plugins-base1.0-dev} \NormalTok{libavutil-dev libavcodec-dev \textbackslash{}}
  \KeywordTok{libcunit1-dev} \NormalTok{libdirectfb-dev xmlto doxygen libxtst-dev}
\KeywordTok{cmake} \NormalTok{-DCMAKE_BUILD_TYPE=Debug -DWITH_SSE2=ON .}
\KeywordTok{make}
\KeywordTok{sudo} \NormalTok{make install}
\KeywordTok{sudo} \NormalTok{echo }\StringTok{"/usr/local/lib/freerdp"} \KeywordTok{>} \NormalTok{/etc/ld.so.conf.d/freerdp.conf}
