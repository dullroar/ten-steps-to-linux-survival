...
have a
\href{https://en.wikipedia.org/wiki/Shebang_\%28Unix\%29}{"shebang"}\index{0@Symbols!\texttt{\#"!} (shebang)}
...
\href{http://linux.die.net/man/1/set}{\texttt{set}}\drcmd{set}\index{Environment variables!Displaying (\texttt{set} command)}
...
Similarly, the
\href{http://linux.die.net/man/1/echo}{\texttt{echo}}\drcmd{echo}\index{Environment variables!Displaying (\texttt{echo} command)}
command can be used to show you the contents of an environment variable
like \texttt{HOME}\drenv{HOME} on \texttt{bash}:
