...
these by using \texttt{less}, redirection and the like. \drcmd{less}
\drnet{ssh}
...
around designing good indexes. My boss, Bryan Henderson, has been
supportive. And finally, I owe more than I can repay (as usual) to my
wife Leslie for putting up with me while I obsessed over this project.
...
\texttt{bash}: \drcmd{set} \drshl{bash} \drshl{CMD.EXE}
...
like \texttt{HOME} on \texttt{bash}: \drcmd{echo}
...
directory is
\href{http://linux.die.net/man/1/ls}{\texttt{ls}}:\drfnd{ls}{list directory contents}
...
remember what \texttt{rm\ -rf\ *}\drfnd{rm}{remove file} means (in the
"UNIX" world it means you're toast if you run it by mistake), thus
...
\def\labelenumi{\arabic{enumi}.}
\item
  \textbf{\texttt{apt-get}}\drpkg{apt-get} - Debian-style package
  manager.
\item
...
\href{http://linux.die.net/man/1/file}{\texttt{file}}\drfnd{file}{detect file type}
...
\href{http://linux.die.net/man/1/cp}{\texttt{cp}}\drfnd{cp}{copy}
command:
...
To \emph{move} use
\href{http://linux.die.net/man/1/mv}{\texttt{mv}}:\drfnd{mv}{move files}
...
the
\href{http://linux.die.net/man/1/rename}{\texttt{rename}}\drfnd{rename}{rename file}
...
\href{http://linux.die.net/man/1/rm}{\texttt{rm}}:\drfnd{rm}{remove file}
...
\href{http://linux.die.net/man/1/touch}{\texttt{touch}}:\drfnd{touch}{change modified date or create file}
...
\texttt{touch}\drfnd{touch}{change modified date or create file} on the
same file again:
...
\href{http://linux.die.net/man/1/mkdir}{\texttt{mkdir}}:\drfnd{mkdir}{make directory}
...
\href{http://linux.die.net/man/1/cd}{\texttt{cd}}\drfnd{cd}{change directory}
...
\href{http://linux.die.net/man/1/pwd}{\texttt{pwd}}\drfnd{pwd}{print working directory}
...
directory, simply use \texttt{cd}\drfnd{cd}{change directory} with no
parameters:
...
\textbf{Note:} The above assumes you have permissions to
\texttt{cd}\drfnd{cd}{change directory} into \texttt{/home/myuser}. See
the upcoming section on file permissions for more info.
...
Did you notice how both \texttt{mkdir}\drfnd{mkdir}{make directory} and
\texttt{touch}\drfnd{touch}{change modified date or create file} allow
for specifying multiple directory and file names in the same command?
...
\index{compression!zipfiles@\texttt{.zip} files} files, although they
don't tend to be the most common, using the
\href{http://linux.die.net/man/1/zip}{\texttt{zip}}\drcom{zip} and
\href{http://linux.die.net/man/1/unzip}{\texttt{unzip}}\drcom{unzip}
commands:
...
\href{http://linux.die.net/man/1/gzip}{\texttt{gzip}}\drcom{gzip},
\href{http://linux.die.net/man/1/bzip2}{\texttt{bzip2}}\drcom{bzip2} and
\href{http://linux.die.net/man/1/7z}{\texttt{7z}}\drcom{7z} (7-zip)
commands.
...
\texttt{gzip}\drcom{gzip} compression, and the
...
basically "tarred and gzipped", or the equivalent to \texttt{.tar.gz} in
the first example.
...
created with the
\href{http://linux.die.net/man/1/ln}{\texttt{ln}}\drfnd{ln}{link}
command. A "soft link"\index{files and directories!soft links} is
equivalent to a Windows shortcut, and can point to a file or a
...
Can you guess how many \texttt{rm}\drfnd{rm}{remove file} commands it
will take to delete the file containing "This is b."?
...
\href{http://linux.die.net/man/1/df}{\texttt{df}}\drfnd{df}{display file system disk space}
command:
...
the
\href{http://linux.die.net/man/8/mount}{\texttt{mount}}\drsys{mount}{mount file system}
...
about the current working directory shown by
\texttt{pwd}\drfnd{pwd}{print working directory} in the following, and
the effects of the \textbf{\emph{relative paths}} as the sample
progresses:
...
\texttt{find}\drfnd{find}{find files}, have parameters specifically
telling the command whether to follow soft links or not (by default,
\texttt{find} does not - see the next chapter for more).
...
\href{http://linux.die.net/man/1/diff}{\texttt{diff}}\drfnd{diff}{show differences between files}
...
coming back to
\href{http://linux.die.net/man/1/find}{\texttt{find}}.\drfnd{find}{find files}
It can be intimidating to figure out from the documentation, especially
at first, but once you start mastering it, you end up using it over and
over again.
...
\href{http://linux.die.net/man/1/grep}{\texttt{grep}}\drfnd{grep}{search files}
...
\texttt{find}\drfnd{find}{find files} instead.
...
\item
  ...
  an interactive session (so it can be ard to distinguish when
...
\emph{stdout}. When a \texttt{bash} script calls
\texttt{echo}\drcmd{echo}, it too is writing to \emph{stdout}. When a
command writes an error message, it is writing to \emph{stderr}. If a
...
so it will read from \emph{stdin} (a quite common "UNIX" command
convention), and echo each line typed by the user to \emph{stdout} until
the "end of file," which in an interactive session can be emulated with
...
is illustrative of redirecting a file to \emph{stdin} for a command or
program: \drstd{stdin}
...
file descriptors: \drstd{stdout} \drstd{stderr}
...
\href{http://linux.die.net/man/1/wget}{\texttt{wget}} \drnet{wget}. Both
...
You see both used often in install scripts that download bits from the
internet and then execute them by piping them to \drcmd{bash}:
...
\href{http://linux.die.net/man/1/ps}{\texttt{ps}}\drsys{ps}{list processes}:
...
\item
  \textbf{\texttt{\textbar{}}} - pipe \emph{stdout} from
  \texttt{grep}\drfnd{grep}{search files} to next command.
\item
...
\href{http://linux.die.net/man/1/top}{\texttt{top}}\drsys{top}{list processes by resource use}
...
like device driver issues. It can also be displayed directly with the
\href{http://linux.die.net/man/8/dmesg}{\texttt{dmesg}}\drsys{dmesg}{display kernel log}
command. The other is \texttt{messages},
...
of course you can \texttt{man\ man}! \drdoc{man} \drcmd{less}
\index{pagination!\texttt{less} command}
\index{files and directories!paginate!less@\texttt{less} command}
...
\href{http://linux.die.net/man/1/passwd}{\texttt{passwd}}\drsys{passwd}{change password}
command and a \texttt{passwd}
...
managers, including \texttt{dpkg}\drpkg{dpkg} and
\texttt{apt-get}\drpkg{apt-get} on Debian-based distros,
\texttt{rpm}\drpkg{rpm} on Fedora descendants, etc. For the rest of this
...
In Debian flavors,
\href{http://linux.die.net/man/8/apt-get}{\texttt{apt-get}}\drpkg{apt-get}
is usually the tool of choice for package management. Another option is
\href{http://linux.die.net/man/8/aptitude}{\texttt{aptitude}}\drpkg{aptitude}.
...
\texttt{apt-get\ dist-upgrade}\drpkg{apt-get}. There is also an
...
You can also \texttt{apt-get\ remove}\drpkg{apt-get} or
\texttt{apt-get\ purge} packages. See the \texttt{apt-get} \texttt{man}
page for details.
...
\texttt{apt-get\ upgrade}\drpkg{apt-get} isn't going to help you here.
This is true no matter what way you get the alternative package -
\texttt{.deb} file, tarball, source code, or whatever (although
\texttt{apt-get} can work with PPAs in a more automated manner).
...
One way is with the
\href{http://linux.die.net/man/1/locate}{\texttt{locate}}\drfnd{locate}{locate files}
command:
...
\href{http://linux.die.net/man/1/pwd}{\texttt{pwd}}\drfnd{pwd}{print working directory}
shows) as part of the path unless \texttt{.} is explicitly listed in
...
\href{http://linux.die.net/man/8/cron}{\texttt{cron}}\drsys{cron}{run scheduled jobs}
in Linux. It reads in the various
...
\href{http://linux.die.net/man/1/crontab}{\texttt{crontab(1)}}\drsys{crontab}{edit scheduled jobs}
...
\href{http://linux.die.net/man/8/reboot}{\texttt{reboot}}\drsys{reboot}{reboot system}
command:
...
\href{http://linux.die.net/man/8/shutdown}{\texttt{shutdown}}\drsys{shutdown}{shutdown or reboot system}
...
\begin{itemize}
\item
  \textbf{\emph{stderr}}\drstd{stderr} - file descriptor 2, always open
  for writing from a process, defaults to the screen on a terminal
  session.
\item
  \textbf{\emph{stdin}}\drstd{stdin} - file descriptor 0, always open
  for reading in a process, defaults to the keyboard input on a terminal
  session.
\item
  \textbf{\emph{stdout}}\drstd{stdout} - file descriptor 1, always open
  for writing from a process, defaults to the screen on a terminal
  session.
\item
  \textbf{\texttt{\textless{}}}\index{*@\texttt{<} (input redirection)}
  \index{I/O!redirection!input (\texttt{<})} - redirect a file to
  \emph{stdin}. \drstd{stdin}
\item
  \textbf{\texttt{\textgreater{}}}\index{*@\texttt{>} (output redirection)}
  \index{I/O!redirection!output (\texttt{>})} - redirect \emph{stdout}
  to a file. \drstd{stdout}
\item
  \textbf{\texttt{2\textgreater{}}}\index{*@\texttt{2>} (\textit{stderr} redirection)}
  \index{I/O!redirection!error (\texttt{2>})} - redirect \emph{stderr}
  to a file. \drstd{stderr}
\item
  \textbf{\texttt{\textbar{}}}\index{*@\texttt{"|} (pipe)}\index{I/O!redirection!pipe (\texttt{"|})}
  - pipe \emph{stdout} from one process into \emph{stdin} in another
  process. \drstd{stdin} \drstd{stdout}
\end{itemize}
...
\begin{itemize}
\item
  \href{http://linux.die.net/man/1/7z}{\textbf{\texttt{7z}}}\drcom{7z} -
...
\item
  \href{http://linux.die.net/man/1/bzip2}{\textbf{\texttt{bzip2}}}\drcom{bzip2}
  - compress and uncompress files using the \texttt{bzip2} algorithm.
\item
  \href{http://linux.die.net/man/1/cat}{\textbf{\texttt{cat}}}\drcmd{cat}
  - concatenate the input files to \emph{stdout}. \drstd{stdout}
\item
  \href{http://linux.die.net/man/1/cd}{\textbf{\texttt{cd}}}\drfnd{cd}{change directory}
...
\item
  \href{http://linux.die.net/man/1/cp}{\textbf{\texttt{cp}}}\drfnd{cp}{copy}
  - copy files or directories.
\item
  \href{http://linux.die.net/man/1/crontab}{\textbf{\texttt{crontab}}}\drsys{crontab}{edit scheduled jobs}
  - display or edit tasks to be run by \texttt{cron}.
\item
  \href{http://linux.die.net/man/1/curl}{\textbf{\texttt{curl}}}\drnet{curl}
  - download files from the internet.
\item
  \href{http://linux.die.net/man/1/df}{\textbf{\texttt{df}}}\drfnd{df}{display file system disk space}
  - show space utilization by file system.
\item
  \href{http://linux.die.net/man/1/diff}{\textbf{\texttt{diff}}}\drfnd{diff}{show differences between files}
...
\item
  \href{http://linux.die.net/man/1/dpkg}{\textbf{\texttt{dpkg}}}\drpkg{dpkg}
  - package manager for Debian flavors.
\item
  \href{http://linux.die.net/man/1/echo}{\textbf{\texttt{echo}}}\drcmd{echo}
  - display passed parameters to \emph{stdout}. \drstd{stdout}
\item
...
\item
  \href{http://linux.die.net/man/1/file}{\textbf{\texttt{file}}}\drfnd{file}{detect file type}
  - give best guess as to type of file.
\item
  \href{http://linux.die.net/man/1/find}{\textbf{\texttt{find}}}\drfnd{find}{find files}
  - find files based on various conditions and execute actions against
  the results.
\item
  \href{http://linux.die.net/man/1/grep}{\textbf{\texttt{grep}}}\drfnd{grep}{search files}
...
\item
  \href{http://linux.die.net/man/1/less}{\textbf{\texttt{less}}}\drcmd{less}\index{files and directories!paginate!less@\texttt{less} command}\index{pagination!\texttt{less} command}
  - display the file one page at a time on \emph{stdout}. \drstd{stdout}
\item
  \href{http://linux.die.net/man/1/ln}{\textbf{\texttt{ln}}}\drfnd{ln}{link}
  - create hard or soft (shortcut) links.
\item
  \href{http://linux.die.net/man/1/locate}{\textbf{\texttt{locate}}}\drfnd{locate}{locate files}
  - locate files by name.
\item
  \href{http://linux.die.net/man/1/ls}{\textbf{\texttt{ls}}}\drfnd{ls}{list directory contents}
...
\item
  \href{http://linux.die.net/man/1/mkdir}{\textbf{\texttt{mkdir}}}\drfnd{mkdir}{make directory}
...
\item
  ...
  - email client.
\item
  \href{http://linux.die.net/man/1/mv}{\textbf{\texttt{mv}}}\drfnd{mv}{move files}
  - move files or directories.
\item
  \href{http://linux.die.net/man/1/pine}{\textbf{\texttt{pine}}}\drnet{pine}
  - email client.
\item
  \href{http://linux.die.net/man/1/ps}{\textbf{\texttt{ps}}}\drsys{ps}{list processes}
  - list running processes.
\item
  \href{http://linux.die.net/man/1/pwd}{\textbf{\texttt{pwd}}}\drfnd{pwd}{print working directory}
  - print the current (working) directory name.
\item
  \href{http://linux.die.net/man/1/rename}{\textbf{\texttt{rename}}}\drfnd{rename}{rename file}
  - rename files in more complex ways than \texttt{mv} can.
\item
  \href{http://linux.die.net/man/1/rm}{\textbf{\texttt{rm}}}\drfnd{rm}{remove file}
...
\item
  ...
  - sort \emph{stdin} or a file to \emph{stdout}. \drstd{stdin}
  \drstd{stdout}
\item
...
\item
  ...
  - write to a file and \emph{stdout} at the same time. \drstd{stdout}
\item
  \href{http://linux.die.net/man/1/telnet}{\textbf{\texttt{telnet}}}\drnet{telnet}
  - ancient terminal program and protocol.
\item
  \href{http://linux.die.net/man/1/top}{\textbf{\texttt{top}}}\drsys{top}{list processes by resource use}
  - list processes by resource utilization (CPU).
\item
  \href{http://linux.die.net/man/1/touch}{\textbf{\texttt{touch}}}\drfnd{touch}{change modified date or create file}
...
\item
  \href{http://linux.die.net/man/1/uname}{\textbf{\texttt{uname}}}\drsys{uname}{system info}
  - print system info.
\item
  \href{http://linux.die.net/man/1/unzip}{\textbf{\texttt{unzip}}}\drcom{unzip}
...
\item
  ...
  - RDP protocol client.
\item
  \href{http://linux.die.net/man/1/zip}{\textbf{\texttt{zip}}}\drcom{zip}
...
\begin{itemize}
\item
  \href{http://linux.die.net/man/8/apt-get}{\textbf{\texttt{apt-get}}}\drpkg{apt-get}
  - package manager for Debian flavors.
\item
  \href{http://linux.die.net/man/8/aptitude}{\textbf{\texttt{aptitude}}}\drpkg{aptitude}
  - package manager for Debian flavors.
\item
  \href{http://linux.die.net/man/8/cron}{\textbf{\texttt{cron}}}\drsys{cron}{run scheduled jobs}
  - system for running "scheduled tasks."
\item
  \href{http://linux.die.net/man/8/dmesg}{\textbf{\texttt{dmesg}}}\drsys{dmesg}{display kernel log}
  - display kernel log messages.
\item
  \href{http://linux.die.net/man/8/ifconfig}{\textbf{\texttt{ifconfig}}}\drnet{ifconfig}
  - display network (interface) configuration.
\item
  \href{http://linux.die.net/man/8/mount}{\textbf{\texttt{mount}}}\drsys{mount}{mount file system}
  - mount a file system to a specific location.
\item
  \href{http://linux.die.net/man/1/passwd}{\textbf{\texttt{passwd}}}\drsys{passwd}{change password}
  - change password.
\item
  \href{http://linux.die.net/man/8/ping}{\textbf{\texttt{ping}}}\drnet{ping}
  - test for network connectivity to an IP address.
\item
  \href{http://linux.die.net/man/8/reboot}{\textbf{\texttt{reboot}}}\drsys{reboot}{reboot system}
  - restart the system.
\item
  \href{http://linux.die.net/man/8/rpm}{\textbf{\texttt{rpm}}}\drpkg{rpm}
  - package manager for Fedora flavors.
\item
  \href{http://linux.die.net/man/8/shutdown}{\textbf{\texttt{shutdown}}}\drsys{shutdown}{shutdown or reboot system}
...
(with their
\href{http://linux.die.net/man/1/uname}{\texttt{uname\ -rv}}\drsys{uname}{system info}
results):
...
\item
  \href{http://www.freebsd.org/}{\textbf{FreeBSD}}\drbsd{FreeBSD} -
