...
\texttt{vi}\drcmd{vi}\index{Editors!vi@\texttt{vi}} editor (obviously),
and some of the "system admin" commands mentioned later, including
system and service restarts. Use your common sense plus the various
resources for documentation mentioned in this book to make sure you
aren't doing anything destructive to your system, especially in
production.
...
\item
  ...
  landing,
  \href{https://en.wikipedia.org/wiki/Multics}{Multics}\drios{Multics}
...
\item
  ...
  as Bell Labs UNIX, System V evolved with subtle and not so subtle
...
\item
  ...
  \item
    \textbf{Sun SunOS}\drios{SunOS} - BSD flavor.
  \item
    \textbf{Sun Solaris}\drios{Solaris} - System V flavor. Now Oracle
    Solaris.
  \item
...
mean POSIX-compliant code will run there unchanged.
...
\begin{itemize}
\item
  \textbf{\texttt{apt-get}\drcmd{apt-get}\index{Package management!\texttt{apt-get} command},
  \texttt{dpkg}\drcmd{dpkg}\index{Package management!\texttt{dpkg} command}
  and
  \texttt{.deb}\index{Files and Directories!\texttt{.deb} package files}
  files} -
...
\item
  \textbf{\texttt{pacman}}\drcmd{pacman}\index{Package management!\texttt{pacman} command}
  - \href{https://en.wikipedia.org/wiki/Arch_Linux}{Arch}\drios{Arch}
  flavors.
\item
  \textbf{\texttt{rpm}\drcmd{rpm}\index{Package management!\texttt{rpm} command}
  and
  \texttt{yum}\drcmd{yum}\index{Package management!\texttt{yum} command}}
  - Red Hat\drios{Red Hat} flavors,such as
  ...
  Enterprise}\drios{Red Hat Enterprise Linux (RHEL)} and
...
\index{Documentation!help@\texttt{help} command} command in a
\texttt{bash} terminal:
...
\index{Environment variables!Displaying (\texttt{set} command)} command
...
\index{Environment variables!Displaying (\texttt{echo} command)} command
can be used to show you the contents of an environment variable like
\texttt{HOME}\drenv{HOME} on \texttt{bash}:
...
\texttt{USERNAME}\drenv{USERNAME} variable, and some like Mint set both.
I think it is better to use \texttt{whoami}, which tends to be on almost
all systems.
...
\texttt{PATH}\drenv{PATH} environment variable under \texttt{bash}:
...
Echoing the \texttt{PATH}\drenv{PATH} environment variable under
\texttt{CMD.EXE} works, too:
...
and \texttt{/}\index{0@Symbols!\texttt{/} (path separator)} for Linux.
But Linux has no concept of a "drive letter" like \texttt{C:} in
Windows, and instead everything is mounted in a single namespace
hierarchy starting at the root \texttt{/}. We'll be talking more about
directories, paths and file systems in the next chapter.

Just to muddy the waters further, notice how Cygwin\drios{Cygwin} under
Windows shows the \texttt{PATH}\drenv{PATH} environment variable with
...
manager like GNOME or KDE, or \texttt{ssh}'ing\drcmd{ssh} into an
interactive session of a remote machine, or even running a
Cygwin\drios{Cygwin} command prompt under Windows.
...
or "dot" files in your home directory called
\texttt{.bash\_history}\index{Files and Directories!Special!\texttt{.bash\_history}},
...
\texttt{/}\index{0@Symbols!\texttt{/} (root directory)}\index{Files and Directories!Root (\texttt{/})}:
...
\texttt{bash}) that looks to install and configure
...
\index{Files and Directories!Paginate (\texttt{less} command)} command
(which is a derivative of the original
\href{http://linux.die.net/man/1/more}{\texttt{more}}\drcmd{more}
\index{Files and Directories!Paginate (\texttt{more} command)}, hence
...
The next example shows the last 10 lines of the kernel
\texttt{dmesg}\index{Files and Directories!Special!\texttt{/var/log/dmesg}}
log:
...
\index{Files and Directories!Detecting type} command to help us guess:
...
talked about
\texttt{pipes}\index{0@Symbols!\texttt{"|} (pipe)}\index{Files and Directories!Redirection}
...
\index{Files and Directories!Current (\texttt{pwd} command)} simply
\emph{prints the working (current) directory}. If \texttt{whoami} tells
you who you are, \texttt{pwd} tells you \textbf{\emph{where}} you are.
...
The tilde
(\texttt{\textasciitilde{}})\index{0@Symbols!\texttt{\~{}} (home directory)}
...
permissions\index{Files and Directories!Permissions}\index{Files and Directories!Permissions}
that can be thought of as a "grid" of 3x3 showing "who has what?" The
"who" is known as "UGO":
...
\texttt{/etc/init.d}\index{Files and Directories!Special!\texttt{/etc/init.d}}
...
\texttt{.zip}\index{Files and Directories!\texttt{.zip} files} files,
...
\texttt{.tar}\index{Files and Directories!\texttt{.tar} files} file.
...
\texttt{.tgz}\index{Files and Directories!\texttt{.tgz} files} file
suffix means basically "tarred and gzipped", or the equivalent to
...
\texttt{/}\index{0@Symbols!\texttt{/} (root directory)}
...
(\texttt{\textbackslash{}})\index{0@Symbols!\texttt{\textbackslash} (escape character)}
...
\item
  ...
  \texttt{*}\index{0@Symbols!\texttt{*} (wildcard)} for \texttt{find} to
  use as it recursively searches through directories, instead of the
  shell expanding it to all files that end in \texttt{.dat} in the
  current directory.
\item
...
\item
  ...
  \texttt{;}\index{0@Symbols!\texttt{;} (command separator)} is normally
  used to separate independent commands on the same line in the shell.
\end{enumerate}
...
\texttt{"\%p\textbackslash{}n\%u\textbackslash{}n\%TY-\%Tm-\%TdT\%TT\textbackslash{}n\textbackslash{}n"}\index{0@Symbols!\texttt{\%} (format)}
breaks down into:
...
\index{0@Symbols!\texttt{*} (match zero or more characters)} character
from the \texttt{CMD.EXE}\drshl{CMD.EXE} prompt and Windows Explorer. It
means "match zero or more characters." We'll cover more on regular
...
In this case the
\texttt{\textgreater{}\ hw}\index{0@Symbols!\texttt{>} (output redirection)}
...
The
\texttt{2\textgreater{}/tmp/finderrors.log}\index{0@Symbols!\texttt{2>} (\textit{stderr} redirection)}
...
\texttt{\textgreater{}\textgreater{}}\index{0@Symbols!\texttt{>>} (output redirection, appending)}
...
\href{http://linux.die.net/man/1/vi}{\texttt{vi}}\drcmd{vi}
\index{Commands!vi@\texttt{vi}|seealso{\texttt{vi} Commands}} stands for
...
i\textbf{m}proved"),\index{Editors!vim@\texttt{vim}} that is then
accessed via the alias \texttt{vi}. The differences tend to be minor,
with \texttt{vim}\drcmd{vim} being more customizable.

\texttt{vi} and a similar editor,
\href{http://linux.die.net/man/1/emacs}{\texttt{emacs}}\drcmd{emacs},
\index{Editors!emacs@\texttt{emacs}} both tend to trip up users from GUI
operating systems such as Windows or OS X that have editors like Notepad
that are always ready for user input.
...
\texttt{view}{]}(http://linux.die.net/man/1/view)\drcmd{view}\index{Editors!view@\texttt{view}},
the alias for \texttt{vi} invoked in read-only mode.
...
\item
  ...
  \index{0@Symbols!\texttt{\$} (\texttt{vi} jump to end of line)} -jumps
...
\item
  \textbf{\texttt{/foo}}\index{vi Commands@\texttt  {vi} Commands!\/@\texttt{/} (Find forward)}
  \index{0@Symbols!\texttt{/} (\texttt{vi} find forward)} - find "foo"
  going forward toward the end of the file.
\item
  \textbf{\texttt{?foo}}\index{vi Commands@\texttt  {vi} Commands!\?@\texttt{?} (Find backward)}
  \index{0@Symbols!\texttt{?} (\texttt{vi} find backward)} - find "foo"
  going backward toward the front of the file.
\item
...
\index{0@Symbols!\texttt{"!} (\texttt{vi} invoke external command)},
...
There may be
\href{http://linux.die.net/man/1/emacs}{\texttt{emacs}}\drcmd{emacs}
\index{Editors!emacs@\texttt{emacs}}, which \textbf{\emph{will}} make
you yearn for the simplicity of \texttt{vi}.
...
\href{http://linux.die.net/man/1/pico}{\texttt{pico}}\drcmd{pico}\index{Editors!pico@\texttt{pico}}
and its successor,
\href{http://linux.die.net/man/1/nano}{\texttt{nano}}\drcmd{nano}
\index{Editors!nano@\texttt{nano}}. You can see the difference the
second you see a file open in \texttt{nano} - in this case, the
...
\href{http://linux.die.net/man/1/wget}{\texttt{wget}} drcmd\{wget\}.
...
\index{Files and Directories!Special!\texttt{/proc/}} "file system." On
modern Linux systems, there is typically a \texttt{/proc} directory that
looks like directories and files:
...
\index{Files and Directories!Special!\texttt{/var/log/}} Here is a
...
of general interest are \texttt{dmesg}\index{dmesg}
\index{Files and Directories!Special!\texttt{/var/log/dmesg}}, which
holds kernel-level debug messages and usually is useful for debugging
things like device driver issues. The other is
\texttt{messages}\index{Files and Directories!Special!\texttt{/var/log/messages}},
...
\index{Files and Directories!Special!\texttt{/tmp/}}. You can place your
...
in that when the system reboots, \texttt{/tmp} is reset to empty.
...
\href{http://linux.die.net/man/1/man}{\texttt{man}}\drcmd{man}\index{Documentation!man@\texttt{man} command},
\href{http://linux.die.net/man/1/info}{\texttt{info}}\drcmd{info}
...
\index{Files and Directories!Special!\texttt{/etc/passwd}} file format
(for \texttt{/etc/passwd}). By default, \texttt{man\ passwd} will show
you the documentation from the lowest numbered section with a match, in
this case section 1, usually referred to as \texttt{passwd(1)} to
...
\texttt{info}\drcmd{info}
\index{Documentation!info@\texttt{info} command} format, which is
originally from \texttt{emacs}. Here are the results of
\texttt{info\ find}:
...
\texttt{apropos}\drcmd{apropos}
\index{Documentation!apropos@\texttt{apropos} command} command can
...
\texttt{/etc}\index{Files and Directories!Special!\texttt{/etc/}}.
...
\begin{itemize}
\item
  \textbf{\texttt{fstab}}\index{Files and Directories!Special!\texttt{/etc/fstab}}
  - a listing of the file systems currently mounted and their types.
\item
  \textbf{\texttt{group}}\index{Files and Directories!Special!\texttt{/etc/group}}
  - the security groups on the system.
\item
  \textbf{\texttt{hosts}}\index{Files and Directories!Special!\texttt{/etc/hosts}}
  - network aliases (overrides DNS, takes effect immediately).
\item
  \textbf{\texttt{init.d}}\index{Files and Directories!Special!\texttt{/etc/init.d/}}
  - startup and shutdown scripts for "services."
\item
  \textbf{\texttt{mtab}}\index{Files and Directories!Special!\texttt{/etc/mtab}}
  - list of current "mounts."
\item
  \textbf{\texttt{passwd}}\index{Files and Directories!Special!\texttt{/etc/passwd}}
  - "shadow" file containing all the user accounts on the system.
\item
  \textbf{\texttt{resolv.conf}}\index{Files and Directories!Special!\texttt{/etc/resolv.conf}}
  - DNS settings.
\item
  \textbf{\texttt{samba}}\index{Files and Directories!Special!\texttt{/etc/samba/}}
...
\index{Package management!\texttt{dpkg} command} and
\texttt{apt-get}\drcmd{apt-get}
\index{Package management!\texttt{apt-get} command} on Debian-based
distros, \texttt{rpm}\drcmd{rpm}
\index{Package management!\texttt{rpm} command} on Fedora descendants,
etc. For the rest of this section we will use Debian tools, but in
general the concepts and problems are similar for the other toolsets.
...
\index{Files and Directories!\texttt{.deb} package files} files for
Debian systems), in source format requiring it to be built, in a zipped
"tarball," and more.
...
\texttt{crontab}\index{Files and Directories!Special!\texttt{crontab}}
...
libraries! The \texttt{which} command comes in handy here for finding
out where each command is installed.
...
\index{0@Symbols!\texttt{?} (exit status environment variable)}
environment variable. Consider if the file \texttt{foo} exists and
\texttt{bar} does not:
...
\texttt{bash} we have
\texttt{\&\&}\index{0@Symbols!\texttt{\&}\texttt{\&} (logical "and" operator)}
...
\href{http://linux.die.net/man/1/false}{\texttt{false}}\drcmd
...
\item
  ...
  \index{Environment variables!\?@\texttt{?}}
  \index{0@Symbols!\texttt{?} (exit status environment variable)} - the
...
\begin{itemize}
\item
  \textbf{\texttt{\&\&}}\index{0@Symbols!\texttt{\&}\texttt{\&} (logical "and" operator)}
  - execute the second command only if the first command succeeds.
\item
  \textbf{\texttt{\textbar{}\textbar{}}}\index{0@Symbols!\texttt{"|"|} (logical "or" operator)}
...
\item
  ...
  the screen on a terminal session.
\item
  \textbf{\texttt{\textless{}}}\index{0@Symbols!\texttt{<} (input redirection)}
  \index{I/O!Redirection!Input (\texttt{<})} - redirect a file to
  \texttt{stdin}.
\item
  \textbf{\texttt{\textgreater{}}}\index{0@Symbols!\texttt{>} (output redirection)}
  \index{I/O!Redirection!Output (\texttt{>})} - redirect \texttt{stdout}
  to a file.
\item
  \textbf{\texttt{2\textgreater{}}}\index{0@Symbols!\texttt{2>} (\textit{stderr} redirection)}
  \index{I/O!Redirection!Error (\texttt{2>})} - redirect \texttt{stderr}
  to a file.
\item
  \textbf{\texttt{\textbar{}}}\index{0@Symbols!\texttt{"|} (pipe)}\index{I/O!Redirection!pipe (\texttt{"|})}
...
\item
  \textbf{\texttt{.bash\_history}}\index{Files and Directories!Special!\texttt{.bash\_history}}
...
\begin{itemize}
\item
  \textbf{\texttt{/etc}}\index{Files and Directories!Special!\texttt{/etc/}}
  - configuration files location.
\item
  \textbf{\texttt{/home}}\index{Files and Directories!Home (\texttt{/home/})}
  - "home" or user profile directories.
\item
  \textbf{\texttt{/proc}}\index{Files and Directories!Special!\texttt{/proc/}}
...
\item
  \textbf{\texttt{/tmp}}\index{Files and Directories!Special!\texttt{/tmp/}}
  - temporary files location.
\item
  \textbf{\texttt{/var/log}}\index{Files and Directories!Special!\texttt{/var/log/}}
  - log files location.
\end{itemize}
